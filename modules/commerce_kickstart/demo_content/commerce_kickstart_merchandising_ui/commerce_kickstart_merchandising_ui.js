    ...
    attach: function(context, settings) {
      ...
      if (typeof $.fn.bxSlider != 'undefined' && processed == false) {
        ...
        var slider = $('.event-slider', context).bxSlider({
          ...
          mode: 'fade',
          prevText: '<span>' + $('.event-slider .views-row:last .views-field-title').text() + '</span>',
          nextText: '<span>' + $('.event-slider .views-row-2 .views-field-title').text() + '</span>',
          onBeforeSlide: function(currentSlideNumber, totalSlideQty, currentSlideHtmlObject){
            ...
            var leftSlideText = $(currentSlideHtmlObject).parents('.event-slider').find('.views-row-' + (leftSlideNumber + 1) + ':first .views-field-title').text();
            var rightSlideText = $(currentSlideHtmlObject).parents('.event-slider').find('.views-row-' + (rightSlideNumber + 1) + ':first .views-field-title').text();
